document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (token && projectId) {
        try {
            ...
            await fetchAndDisplayTasks(projectId, currentPage, pageSize, token);

            document.getElementById('prev-page').addEventListener('click', async function() {
                if (currentPage > 0) {
                    currentPage--;
                    tasks = await fetchAndDisplayTasks(projectId, currentPage, pageSize, token);
                }
            });
...
async function fetchAndDisplayProjectDetails(projectId, token) {
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('edit-project-button').href = `edit-project.html?projectId=${projectId}`;
            document.getElementById('create-task-button').href = `create-task.html?projectId=${projectId}`;
        } else {
...
async function fetchAndDisplayTasks(projectId, page, size, token) {
    try {
        const response = await fetch(`http://localhost:8080/api/project/${projectId}?page=${page}&size=${size}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
            }
        ...
        if (response.ok) {
            ...
            tasks.forEach(task => {
                ...
                listItem.href = `task.html?taskId=${task.id}&projectId=${projectId}`;
